...
```{r }
ihcData <- "../out/CRLM_IHC_data_parsed.csv"
...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("ngfr_ap", "cd68_dab", "ck18_ap")), aes(x = std_dist_cat, y = intensity_avg, group = markers, colour = markers)) + geom_line() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ggtitle(paste("means"))
