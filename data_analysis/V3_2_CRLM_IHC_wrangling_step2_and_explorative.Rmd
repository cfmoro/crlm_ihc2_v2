...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("ck18_ap", "crp_dab") & std_dist_cat %in% invf_perit_dists), aes(x = std_dist_cat, y = intensity_avg, group = markers, colour = markers)) + 
  ...
  geom_ribbon(aes(ymin = intensity_lower, ymax = intensity_upper, fill = markers), alpha = 0.2, linetype="dashed", color="grey") +
  ...
  annotate(geom="text", x=32, y=0.37, label="CK18", size=6, color="#e19273") +
  annotate(geom="text", x=35, y=0.18, label="CRP", size=6, color="#86b5da")
...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("ngfr_ap", "actinsm_ap")), aes(x = std_dist_cat, y = intensity_avg_roll, group = markers, colour = markers)) +
  ...
  annotate(geom="text", x=8, y=0.05, label="NGFR", size=6, color="#e19273") +
  annotate(geom="text", x=8.5, y=0.21, label="ASMA", size=6, color="#86b5da")
