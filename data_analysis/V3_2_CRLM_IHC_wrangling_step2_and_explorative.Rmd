...
# Calculate the rolling avgeraged intensity
avgStdIhcDataDistLong <- avgStdIhcDataDistLong %>% group_by(markers) %>% arrange (std_dist_cat) %>% mutate(intensity_roll_avg = runmean(intensity_avg, 5, alg = "exact", endrule = "mean", align = "center"))
...
avgRegionIhcDataDistLongV2 <- stdIhcDataDistLong %>% group_by(image, markers, region_part) %>% summarise(intensity_avg = mean(intensity_mean, na.rm = TRUE)) %>% filter(!is.na(markers), !is.na(intensity_avg)) # TODO: Where do those NaN come from?
```
...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("ngfr_ap", "actinsm_ap", "p53_d240_dab")), aes(x = std_dist_cat, y = intensity_roll_avg, group = markers, colour = markers)) +
  geom_rect(aes(xmin=1, xmax=21,ymin=-Inf, ymax=Inf), fill="yellow1", color = NA, alpha = 0.01,  show.legend = FALSE) +
  geom_rect(aes(xmin=21, xmax=41,ymin=-Inf, ymax=Inf), fill="wheat1", color = NA, alpha = 0.01,  show.legend = FALSE) +
  geom_rect(aes(xmin=41, xmax=60,ymin=-Inf, ymax=Inf), fill="darkslategray1", color = NA, alpha = 0.01,  show.legend = FALSE) +  
  geom_line() + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("p53_cd34_dab", "cd68_dab", "cald_ck19/ck7_ap") & std_dist_cat %in% invf_perit_dists), aes(x = std_dist_cat, y = intensity_roll_avg, group = markers, colour = markers)) +
  geom_rect(aes(xmin=1, xmax=21,ymin=-Inf, ymax=Inf), fill="wheat1", color = NA, alpha = 0.01,  show.legend = FALSE) +
  geom_rect(aes(xmin=21, xmax=40,ymin=-Inf, ymax=Inf), fill="darkslategray1", color = NA, alpha = 0.01,  show.legend = FALSE) +    
  geom_line() + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Capillaries, macrophages and ductuli") +
...
ggplot(data = filter(avgStdIhcDataDistLong, markers %in% c("ck18_ap", "crp_dab") & std_dist_cat %in% invf_perit_dists), aes(x = std_dist_cat, y = intensity_roll_avg, group = markers, colour = markers)) + 
  geom_rect(aes(xmin=1, xmax=21,ymin=-Inf, ymax=Inf), fill="wheat1", color = NA, alpha = 0.01,  show.legend = FALSE) +
  geom_rect(aes(xmin=21, xmax=40,ymin=-Inf, ymax=Inf), fill="darkslategray1", color = NA, alpha = 0.01,  show.legend = FALSE) +      
  geom_line() + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Hepatocytes (and ductuli in inv. front)")  +
